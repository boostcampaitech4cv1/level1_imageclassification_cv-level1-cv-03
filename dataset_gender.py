...
class Handkerchief(MaskBaseDataset):
    """
        train / val 나누는 기준을 이미지에 대해서 random 이 아닌
        사람(profile)을 기준으로 나눕니다.
        구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다
    ...
    def setup(self):
        profiles = os.listdir(self.data_dir)   
        profiles = [profile for profile in profiles if not profile.startswith(".") and not profile.startswith("._") and 5400 < int(profile[0:6]) < 5600 ]
        split_profiles = self._split_profile(profiles, self.val_ratio)
        ...
        for phase, indices in split_profiles.items():
            for _idx in indices:
                ...
                for file_name in os.listdir(img_folder):
                    ...
                    if int(profile[0:6]) < 5400 or int(profile[0:6]) >= 5600: 
                        continue
                    
                    if file_name == 'mask5.jpg':
